...
If a linear map maps between two finite-dimensional vector spaces and both of those have a defined basis, it can be represented by a \textbf{matrix}.
...
\textbf{Matrix multiplication} $\cdot: \C^{a \times b} \times \C^{b \times c} \to \C^{a \times c}$ is equivalent to the composition of the linear maps they represent, it is defined as
\begin{align}
...
This is the case exactly when there exists a matrix $A^{-1} \in C^{n \times n}$, such that $A^{-1} \cdot A = I$, where $I$ is the identity matrix. 
...
The following properties of eigenvalues are required later.
